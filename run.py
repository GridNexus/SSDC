...
def get_dist_launch(args):
    if args.dist == 'f4':
        return "CUDA_VISIBLE_DEVICES=4,5,6,7 WORLD_SIZE=4 python3 -m torch.distributed.run --nproc_per_node=4 " \
               "--nnodes={:} --node_rank={:} " \
...
def run(args):
    ...
    if 'base' in task_lower or task_lower in ('cmp', 'ssdc'):
        config_name = 'ssdc' if task_lower in ('cmp', 'ssdc') else args.task
        args.config = 'configs/' + config_name + '.yaml'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task', default='ssdc', type=str)
    parser.add_argument('--dist', default='f4', type=str, help="see func get_dist_launch for details")
    parser.add_argument('--output_dir', default='out/ssdc', type=str, help='local path')
