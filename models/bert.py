...
import inspect # Added for compatibility
...
from transformers.modeling_utils import (
    PreTrainedModel,
    # apply_chunking_to_forward, # REMOVED: Causing ImportError
...
# =================================================================
# ADDED: Missing function from old transformers version
# =================================================================
def apply_chunking_to_forward(
    ...
    assert len(input_tensors) > 0, "If chunk_size is defined, input_tensors should not be empty"
    tensor_shape = input_tensors[0].shape
    assert all(
        input_tensor.shape == tensor_shape for input_tensor in input_tensors
    ), "All input tenors should have the same shape"

    if len(inspect.signature(forward_fn).parameters) != len(input_tensors):
        raise ValueError(
            f"Forward function {forward_fn} expects {len(inspect.signature(forward_fn).parameters)} arguments, but only {len(input_tensors)} were passed."
        )
    ...
    if chunk_size > 0:
        ...
        # chunk input tensor into smaller chunks
        input_tensors_chunks = tuple(input_tensor.chunk(chunk_size, dim=chunk_dim) for input_tensor in input_tensors)
        
        # apply forward_fn to each chunk
        output_chunks = []
        for input_tensors_chunk in zip(*input_tensors_chunks):
            output_chunks.append(forward_fn(*input_tensors_chunk))
