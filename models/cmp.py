...
# Load CMP-level utils module explicitly to avoid conflicts when CMP is imported elsewhere
_CMP_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
_CMP_UTILS_PATH = os.path.join(_CMP_ROOT, 'utils.py')
_spec = importlib.util.spec_from_file_location('cmp_utils', _CMP_UTILS_PATH)
cmp_utils = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(cmp_utils)
...
def build_vision_encoder(config):
    ...
    vision_config = cmp_utils.read_json(config['vision_config'])
